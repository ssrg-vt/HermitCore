...
int uhyve_gdb_init(int vcpufd)
{
     ...
     * replacing instructions with int3's/brks.
     */
...
static struct breakpoint_t *bp_list_find(gdb_breakpoint_type type, uint64_t addr, size_t len)
{
    struct breakpoint_t *bp;
...
int uhyve_gdb_read_registers(int vcpufd,
                            uint8_t *registers,
                            size_t *len)
{
...
int uhyve_gdb_write_registers(int vcpufd,
                             uint8_t *registers,
                             size_t len)
{
    struct uhyve_gdb_regs *gregs = (struct uhyve_gdb_regs *) registers;
